...
# Need to update HTML for Show Answer fix, Hint button, and Listening indicator
...
<head>
    ...
    <style>
        ...
        .pulsating-dot {{
            ...
            box-shadow: 0 0 0 0 rgba(255, 180, 171, 1);
            transform: scale(1);
            animation: pulse 2s infinite;
        }}
        @keyframes pulse {{
            0% {{ transform: scale(0.95); box-shadow: 0 0 0 0 rgba(255, 180, 171, 0.7); }}
            70% {{ transform: scale(1); box-shadow: 0 0 0 10px rgba(255, 180, 171, 0); }}
        ...
        <article class="w-full space-y-8">
            ...
            <div class="bg-surface-container-high p-4 rounded-lg border border-outline-variant/20 text-center sticky bottom-[100px] z-40">
                <div class="flex items-center justify-center gap-2 mb-1">
                    <span id="listening-indicator" class="pulsating-dot hidden"></span>
                    <p class="text-sm text-outline font-label-md uppercase tracking-widest">Live Transcript</p>
                </div>
                <p id="transcript-display" class="text-lg text-on-surface min-h-[1.5rem] italic">Waiting to start...</p>
            </div>
        </article>
        ...
        <div class="w-full space-y-8">
            ...
            <div class="space-y-6 relative">
                ...
                <div class="absolute right-0 top-0 flex gap-2">
                    <button id="btn-hint" onclick="hintTranslation()" class="hint-btn px-4 py-1 flex items-center gap-1 rounded-full font-label-lg transition-all active:scale-95">
                        <span class="material-symbols-outlined text-sm">lightbulb</span> Hint
                    </button>
                    <button id="btn-show-answer-container" onclick="solveTranslation()" class="hidden px-4 py-1 text-sm bg-error-container text-on-error-container rounded-full font-label-lg hover:brightness-110 active:scale-95 transition-all">
                        Show Answer
                    </button>
                </div>
                ...
                <!-- Source Area -->
                <div id="trans-source-area" class="flex flex-wrap justify-center gap-3 p-4 min-h-[100px]">
                </div>
            </div>
    ...
    <script>
        ...
        function renderLevels() {{
            ...
            stories.forEach((story, idx) => {{
                ...
                // Fetch saved progress for translation
                const savedSentenceIndex = parseInt(localStorage.getItem(`panchatantra_trans_prog_${{idx}}`)) || 0;
                let transLabel = "Translate";
                if (savedSentenceIndex > 0) {{
                    if (savedSentenceIndex >= story.sentences.length) {{
                        transLabel = "Translation Complete";
                    }} else {{
                        transLabel = `Translate (Resuming ${{(savedSentenceIndex+1)}}/${{story.sentences.length}})`;
                    }}
                }}
                ...
                card.innerHTML = `
                    ...
                    <div class="p-6 bg-surface-container flex-grow flex flex-col">
                        ...
                        <div class="mt-auto flex flex-col gap-2">
                            ...
                            <button ${{!isUnlocked ? 'disabled' : ''}} onclick="startTranslationMode(${{idx}})" class="px-2 py-2 bg-surface-container-high border border-outline-variant rounded text-on-surface hover:bg-primary-container hover:text-on-primary-container transition-colors disabled:opacity-50 text-xs text-center">${{transLabel}}</button>
                        </div>
                    </div>
                `;
        ...
        function startReadingMode(index) {{
            ...
            document.getElementById('transcript-display').innerText = "Waiting to start...";
        ...
        function updateRecordBtnUI() {{
            ...
            if (isListening) {{
                ...
                if(tDisplay.innerText === "Waiting to start...") tDisplay.innerText = "Listening...";
            }} else {{
        ...
        function startTranslationMode(index) {{
            ...
            const savedSentenceIndex = parseInt(localStorage.getItem(`panchatantra_trans_prog_${{index}}`)) || 0;
            const story = stories[index];

            if (savedSentenceIndex >= story.sentences.length) {{
                showSuccessModal("Already Completed", "You have already translated this entire story!");
                currentSentenceIndex = 0;
                localStorage.setItem(`panchatantra_trans_prog_${{index}}`, 0);
            }} else {{
        ...
        function hintTranslation() {{
            wasHintUsed = true;
            const story = stories[currentStoryIndex];
            const sentence = story.sentences[currentSentenceIndex];
            const correctWords = sentence.hindi.split(" ");

            // Find the first incorrect word in selected array
            let firstWrongIdx = -1;
            for (let i = 0; i < selectedTransWords.length; i++) {{
                if (selectedTransWords[i].word !== correctWords[i]) {{
                    firstWrongIdx = i;
                    break;
                }}
            }}

            // If the selected ones are all correct so far, the next step is just placing the next right word
            if (firstWrongIdx === -1) {{
                firstWrongIdx = selectedTransWords.length;
            }}

            if (firstWrongIdx < correctWords.length) {{
                const neededWord = correctWords[firstWrongIdx];

                // Pop everything from firstWrongIdx onwards back to source
                while (selectedTransWords.length > firstWrongIdx) {{
                    const w = selectedTransWords.pop();
                    sourceTransWords.push(w.word);
                }}

                // Find neededWord in source and select it
                const sourceIdx = sourceTransWords.indexOf(neededWord);
                if (sourceIdx !== -1) {{
                    const word = sourceTransWords.splice(sourceIdx, 1)[0];
                    selectedTransWords.push({{word: word, origIdx: sourceIdx}});
                }}
            }}
        ...
        function solveTranslation() {{
            ...
            // Push correct words into target array
            const correctWords = sentence.hindi.split(" ");
            correctWords.forEach((w, idx) => {{
                selectedTransWords.push({{word: w, origIdx: idx}});
            }});

            renderTranslationUI();
            // We DO NOT automatically advance. The user must click "Check Sentence" to acknowledge it.
        }}
        ...
        function checkTranslation() {{
            ...
            if (currentStr === sentence.hindi) {{
                // Only award full XP if not auto-solved
                if (!wasAutoSolved) {{
                    if (wasHintUsed) {{
                        updateGlobalScore(25); // half xp for hints
                    }} else {{
                        updateGlobalScore(50); // full xp
                    }}
                }}
                ...
                // Save progress
                localStorage.setItem(`panchatantra_trans_prog_${{currentStoryIndex}}`, currentSentenceIndex + 1);
                ...
                failedTranslationAttempts++;
                if (failedTranslationAttempts >= 10) {{
