...
# Need to update HTML to add the failed attempts counter, the 'Show Answer' button, and local storage tracking for completed sentences.
    ...
    <main id="view-translate" class="hidden p-6 md:p-12 max-w-3xl mx-auto flex flex-col items-center min-h-[70vh] justify-center">
        <div class="w-full space-y-8">
            ...
            <div class="space-y-6 relative">
                ...
                <div id="btn-show-answer-container" class="absolute right-0 top-0 hidden">
                    <button onclick="solveTranslation()" class="px-4 py-1 text-sm bg-error-container text-on-error-container rounded-full font-label-lg hover:brightness-110 active:scale-95 transition-all">
                        Show Answer
                    </button>
                </div>
    ...
    <script>
        ...
        function startTranslationMode(index) {{
            ...
            if (savedSentenceIndex >= story.sentences.length) {{
                ...
                // we'll still start them at the beginning if they want to replay, or we could just reset
        ...
        function solveTranslation() {{
            ...
            renderTranslationUI();
            checkTranslation(true); // pass true to indicate it was auto-solved
        }}

        function checkTranslation(autoSolved = false) {{
            const story = stories[currentStoryIndex];
            const sentence = story.sentences[currentSentenceIndex];
